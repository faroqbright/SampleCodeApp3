...
export default JobDetailsHeader = (props) => {
    const [selectedId, setSelectedId] = useState('1');

    let uri= props?.detailParams?.user?.company_logo
    return (
        ...
        <Image
                style={{width:48, height:48, marginLeft:20, marginTop:25, }}
                source={uri?{uri:uri}:Images.companyImagePlaceHolder}
            />
            <View style={styles.CompanyNameContainer}>
                <Text style={styles.itemText}>{props?.detailParams?.user?.organization_name?props?.detailParams?.user?.organization_name:'Company Name'}</Text>
        ...
        <View style={styles.textContainer}>
            <View style={styles.salaryLocationContainer}>
                ...
                <Text style={styles.location}>{props?.detailParams?.user?.city?props?.detailParams?.user?.city:'City'}{','} {props?.detailParams?.location?props?.detailParams?.location:'Country'}</Text>
            </View>
        </View>
        <View>
